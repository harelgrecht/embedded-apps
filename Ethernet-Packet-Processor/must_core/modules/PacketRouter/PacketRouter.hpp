...
#include "../PacketAuthentication/PacketAuthentication.hpp"
...
class PacketRouter {
    public:
        PacketRouter(ThreadSafeQueue<T>& receiveQueue, ThreadSafeQueue<T>& toSendProcessQueue, ThreadSafeQueue<T>& fromTunnelQueue, ThreadSafeQueue<T>& guiQueue);
...
PacketRouter<T>::PacketRouter(ThreadSafeQueue<T>& receiveQueue, ThreadSafeQueue<T>& processQueue, ThreadSafeQueue<T>& fromTunnelQueue, ThreadSafeQueue<T>& guiQueue) 
    : receiveQueue_(receiveQueue), toSendProcessQueue_(processQueue), fromTunnelQueue_(fromTunnelQueue), guiQueue_(guiQueue) {
    ...
    // Extract values from the JSON object
        ...
        GUI_PAYLOAD_ICD_ = jsonConfig.at("GUI_PAYLOAD_ICD").get<uint16_t>();
...
void PacketRouter<T>::packetRouteHandler() {
    std::vector<T> rawPacket;    
    while(true) {
        std::vector<T> rawPacket = receiveQueue_.dequeue();
        PacketInfo packet = parsePacket(rawPacket);
        
        if(PacketAuthentication::authenticatePacket(packet.payload)){
            if (isFromTunnel(packet.srcIP, packet.destIP)) { 
                if(isFromGui(packet)) {
                    guiQueue_.enqueue(rawPacket);
                } else
                    fromTunnelQueue_.enqueue(rawPacket);
            } else {
                toSendProcessQueue_.enqueue(rawPacket);
            }   
        }
    } 
...
bool PacketRouter<T>::isFromTunnel(const std::string& srcIP, const std::string& destIP) {
    return (tunnelDevice_.isIpInDestList(srcIP)) && (destIP == tunnelDevice_.getSelfIp());
}
...
template <typename T>
PacketInfo PacketRouter<T>::parsePacket(const std::vector<T>& packet) {
