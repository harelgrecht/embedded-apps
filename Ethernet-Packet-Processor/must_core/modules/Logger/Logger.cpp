#include "Logger.hpp"

Logger::Logger(const std::string& logPath) : logPath_(logPath), logFile_(logPath, std::ios::app) {
    if (!logFile_.is_open()) {
        throw std::runtime_error("Failed to open log file: " + logPath_);
    }
}

Logger::~Logger() {
    if (logFile_.is_open()) {
        logFile_.close();
    }
}

std::string Logger::getSystemUptime() const {
    struct sysinfo info;
    if (sysinfo(&info) != 0) {
        std::cerr << "[Logger] sysinfo call failed" << std::endl;
        return "00:00:00"; // Default value if sysinfo fails
    }

    unsigned long seconds = info.uptime;
    unsigned int hours = seconds / SECONDS_PER_HOUR;
    unsigned int minutes = (seconds % SECONDS_PER_HOUR) / SECOND_PER_MINUTE;
    unsigned int secs = seconds % SECOND_PER_MINUTE;

    std::ostringstream uptimeStream;
    uptimeStream << std::setw(DIGITS_FOR_FORMAT) << std::setfill('0') << hours << ":"
                    << std::setw(DIGITS_FOR_FORMAT) << std::setfill('0') << minutes << ":"
                    << std::setw(DIGITS_FOR_FORMAT) << std::setfill('0') << secs;
    
    return uptimeStream.str();
}

void Logger::Log(LogLevel level, const std::string& message) {
    const std::string uptime = getSystemUptime();
    std::lock_guard<std::mutex> lock(logMutex_); 
    if (logFile_.is_open()) {
        std::ostringstream oss;
        oss << "[" << uptime << "] [" 
            << levelStr[static_cast<int>(level)] << "] " 
            << message << std::endl;

        logFile_ << oss.str();
        logFile_.flush();
    } else {
        std::cerr << "[Logger] Error writing to log file: " << logPath_ << std::endl;
    }
    #ifdef PRINT_TO_SCREEN
        std::cout << "[" << uptime << "] [" 
                  << levelStr[static_cast<int>(level)] << "] " 
                  << message << std::endl;
    #endif
}

Logger& Logger::getInstance(const std::string& logPath) {
    std::call_once(onceFlag_, [logPath]() {
        instance_.reset(new Logger(logPath));
    });
    return *instance_;
}

Logger& Logger::getInstance() {
    return *instance_;
}