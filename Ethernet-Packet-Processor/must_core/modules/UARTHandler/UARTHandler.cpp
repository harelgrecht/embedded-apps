...
bool UARTHandler::configureUART() {
    ...
    if (uartFD_ == -1) {
        Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] Error opening UART device: " + device_);
        return false;
    }

    struct termios options;
    if (tcgetattr(uartFD_, &options) != 0) {
        Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] Error getting UART attributes");
        close(uartFD_);
        return false;
    }
    ...
    if (tcsetattr(uartFD_, TCSANOW, &options) != 0) {
        Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] Failed to apply UART device settings");
        close(uartFD_);
        return false;
    }
    return true;
...
bool UARTHandler::sendData(const std::string& data) {
    ...
    if (uartFD_ == -1) {
        Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] UART is not open");
        return false;
    }
    ssize_t bytes_written = write(uartFD_, data.c_str(), data.length());
    if (bytes_written < 0) {
        Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] Error sending data");
        return false;
    }
    return true;
}
...
void UARTHandler::receiveLoop() {
    ...
    while (run_receive_thread_.load()) {
        ...
        if (readBytes > 0) {
            ...
            Logger::getInstance().Log(LogLevel::ERROR, "[UARTHandler] Error reading data");
            break;
        }
    }
}
